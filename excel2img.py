...
def export_imgs(fn_excel, fn_images, page, _ranges):
    """ Exports images from excel file """

    if len(fn_images)!=len(_ranges):
        raise Exception("Number of images and ranges should be the same")

    with ExcelFile.open(fn_excel) as excel:
        for i in range(len(fn_images)):
            fn_image = fn_images[i]
            _range = _ranges[i]
             # if both page and page-less range are specified, concatenate them into range
            if _range is not None and page is not None and '!' not in _range:
                _range = "'%s'!%s"%(page, _range)
            ...
            while not success:
                try:
                    ...
                    # When other (big) Excel documents are open CopyPicture fails intermittently
                    retries -= 1
                    #print("CopyPicture failed, retries left:"+str(retries))
                    if retries == 0: raise
            print('Created file '+fn_image)
            #time.sleep(1)
